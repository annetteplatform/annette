...
private[impl] class PostIndexEventProcessor(
  ...
  def buildHandler(): ReadSideProcessor.ReadSideHandler[PostEntity.Event] =
    readSide
      ...
      .setEventHandler[PostEntity.PostIntroUpdated](handle(indexDao.updatePostIntro))
      .setEventHandler[PostEntity.PostContentUpdated](handle(indexDao.updatePostContent))
