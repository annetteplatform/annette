  ...
  def run(): Future[Done] =
    ConfigSource.default
      ...
      .fold(
        ...
        config =>
          if (config.enable) run(config)
          else Future.successful(Done)
      )

  private def run(config: InitAuthorizationConfig): Future[Done] = {
    val roles                  = config.roles.map {
      case (id, role) =>
        role
          .into[CreateRolePayload]
          .withFieldConst(_.id, id)
          .withFieldConst(_.createdBy, config.createdBy)
          .transform
    }
    val assignments            = config.assignments.map { assignment =>
    ...
    tryInit(promise, roles, assignments, 100)
    promise.future
  }
