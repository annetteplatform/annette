...
class CategoryEntityService(
  ...
  private def convertSuccess(id: CategoryId, confirmation: CategoryEntity.Confirmation): Done =
    confirmation match {
      ...
      case CategoryEntity.NotFound     => throw PrincipalGroupCategoryNotFound(id)
      case CategoryEntity.AlreadyExist => throw PrincipalGroupCategoryAlreadyExist(id)
  ...
  def getCategoryById(id: CategoryId): Future[Category] =
    refFor(id)
      ...
      .map {
        ...
        case _                                      => throw PrincipalGroupCategoryNotFound(id)
      }
  ...
  def getCategoryByIdFromReadSide(id: CategoryId): Future[Category] =
    for {
      ...
      case None           => throw PrincipalGroupCategoryNotFound(id)
    }
