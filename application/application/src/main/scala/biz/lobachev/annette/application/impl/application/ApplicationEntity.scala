...
object ApplicationEntity {
  ...
  def apply(entityContext: EntityContext[Command]): Behavior[Command] =
    apply(PersistenceId(entityContext.entityTypeKey.name, entityContext.entityId))
      .withTagger {
        case evt: ApplicationCreated                 => Set(tagger.tagFor(evt.id))
        case evt: ApplicationNameUpdated             => Set(tagger.tagFor(evt.id))
        case evt: ApplicationIconUpdated             => Set(tagger.tagFor(evt.id))
        case evt: ApplicationLabelUpdated            => Set(tagger.tagFor(evt.id))
        case evt: ApplicationLabelDescriptionUpdated => Set(tagger.tagFor(evt.id))
        case evt: ApplicationTranslationsUpdated     => Set(tagger.tagFor(evt.id))
        case evt: ApplicationBackendUrlUpdated       => Set(tagger.tagFor(evt.id))
        case evt: ApplicationFrontendUrlUpdated      => Set(tagger.tagFor(evt.id))
        case evt: ApplicationDeleted                 => Set(tagger.tagFor(evt.id))
      }
