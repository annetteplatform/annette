...
private[impl] class LanguageIndexEventProcessor(
  ...
  override val projectionName: String = "language-indexing"
  ...
  override def process(envelope: EventEnvelope[LanguageEntity.Event]): Future[Done] =
    envelope.event match {
      case evt: LanguageEntity.LanguageCreated => indexDao.createLanguage(evt).map(_ => Done)
      case evt: LanguageEntity.LanguageUpdated => indexDao.updateLanguage(evt).map(_ => Done)
      case evt: LanguageEntity.LanguageDeleted => indexDao.deleteLanguage(evt).map(_ => Done)
