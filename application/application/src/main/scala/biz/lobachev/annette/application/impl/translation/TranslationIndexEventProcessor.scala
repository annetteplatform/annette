...
private[impl] class TranslationIndexEventProcessor(
  ...
  override val projectionName: String = "translation-indexing"
  ...
  override def process(envelope: EventEnvelope[TranslationEntity.Event]): Future[Done] =
    envelope.event match {
      case evt: TranslationEntity.TranslationCreated => indexDao.createTranslation(evt).map(_ => Done)
      case evt: TranslationEntity.TranslationUpdated => indexDao.updateTranslation(evt).map(_ => Done)
      case evt: TranslationEntity.TranslationDeleted => indexDao.deleteTranslation(evt).map(_ => Done)
