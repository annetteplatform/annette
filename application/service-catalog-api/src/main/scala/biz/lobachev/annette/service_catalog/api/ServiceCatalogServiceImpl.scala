...
import biz.lobachev.annette.service_catalog.api.service.{
  ActivateServicePayload,
  CreateServicePayload,
  DeactivateServicePayload,
...
class ServiceCatalogServiceImpl(api: ServiceCatalogServiceApi) extends ServiceCatalogService {
  ...
  override def activateService(payload: ActivateServicePayload): Future[Done] =
    api.activateService.invoke(payload)

  override def deactivateService(payload: DeactivateServicePayload): Future[Done] =
    api.deactivateService.invoke(payload)

  override def deleteService(payload: DeleteServicePayload): Future[Done] =
    api.deleteService.invoke(payload)

  override def getServiceById(id: ServiceId, fromReadSide: Boolean): Future[Service] =
    api.getServiceById(id, fromReadSide).invoke()

  override def getServicesById(ids: Set[ServiceId], fromReadSide: Boolean): Future[Seq[Service]] =
    api.getServicesById(fromReadSide).invoke(ids)

  override def findServices(payload: FindServiceQuery): Future[FindResult] =
