...
object ServiceItemEntity {
  ...
  def apply(entityContext: EntityContext[Command]): Behavior[Command] =
    apply(PersistenceId(entityContext.entityTypeKey.name, entityContext.entityId))
      .withTagger {
        case evt: GroupCreated           => Set(tagger.tagFor(evt.id))
        case evt: GroupUpdated           => Set(tagger.tagFor(evt.id))
        case evt: ServiceCreated         => Set(tagger.tagFor(evt.id))
        case evt: ServiceUpdated         => Set(tagger.tagFor(evt.id))
        case evt: ServiceItemActivated   => Set(tagger.tagFor(evt.id))
        case evt: ServiceItemDeactivated => Set(tagger.tagFor(evt.id))
        case evt: ServiceItemDeleted     => Set(tagger.tagFor(evt.id))
      }
