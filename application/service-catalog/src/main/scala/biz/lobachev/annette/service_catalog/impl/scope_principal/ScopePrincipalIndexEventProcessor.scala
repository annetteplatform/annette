...
private[impl] class ScopePrincipalIndexEventProcessor(
  ...
  override val projectionName: String = "scopePrincipal-indexing"
  ...
  override def process(envelope: EventEnvelope[ScopePrincipalEntity.Event]): Future[Done] =
    envelope.event match {
      case evt: ScopePrincipalEntity.ScopePrincipalAssigned   => indexDao.assignPrincipal(evt).map(_ => Done)
      case evt: ScopePrincipalEntity.ScopePrincipalUnassigned => indexDao.unassignPrincipal(evt).map(_ => Done)
