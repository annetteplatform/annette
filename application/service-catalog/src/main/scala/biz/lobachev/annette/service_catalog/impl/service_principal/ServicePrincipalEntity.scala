...
object ServicePrincipalEntity {
  ...
  def apply(entityContext: EntityContext[Command]): Behavior[Command] =
    apply(PersistenceId(entityContext.entityTypeKey.name, entityContext.entityId))
      .withTagger {
        case evt: ServicePrincipalAssigned   => Set(tagger.tagFor(s"${evt.serviceId}~${evt.principal}"))
        case evt: ServicePrincipalUnassigned => Set(tagger.tagFor(s"${evt.serviceId}~${evt.principal}"))
      }
