...
package biz.lobachev.annette.service_catalog.service.category.dao
...
import biz.lobachev.annette.service_catalog.service.category
...
class CategoryIndexDao(client: ElasticClient, configPath: String)(implicit
  ...
  def createCategory(event: category.CategoryEntity.CategoryCreated) =
    createIndexDoc(
      event.id,
      "id"        -> event.id,
      "name"      -> event.name,
  ...
  def updateCategory(event: category.CategoryEntity.CategoryUpdated) =
    updateIndexDoc(
      event.id,
      "id"        -> event.id,
      "name"      -> event.name,
  ...
  def deleteCategory(event: category.CategoryEntity.CategoryDeleted) =
