...
import biz.lobachev.annette.service_catalog.service.group.GroupEntity
import biz.lobachev.annette.service_catalog.service.group.GroupEntity.GroupDeleted
...
class GroupIndexDao(client: ElasticClient)(implicit
  override val ec: ExecutionContext
) extends AbstractIndexDao(client) {

  override val log = LoggerFactory.getLogger(this.getClass)

  override def indexConfigPath = "indexing.group-index"
  ...
  def activateGroup(event: GroupEntity.GroupActivated) = {
    val doc = List(
      "id"        -> event.id,
      "active"    -> true,
  ...
  def deactivateGroup(event: GroupEntity.GroupDeactivated) = {
    val doc = List(
      "id"        -> event.id,
      "active"    -> false,
      "updatedAt" -> event.updatedAt
  ...
  def deleteGroup(event: GroupDeleted) =
