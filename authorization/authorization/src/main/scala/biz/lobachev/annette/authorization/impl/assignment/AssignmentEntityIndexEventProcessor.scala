...
// Replaces the Lagom-variant AssignmentEntityIndexEventProcessor (extends ReadSideProcessor[Event]).
private[impl] class AssignmentEntityIndexEventProcessor(
  ...
  override val projectionName: String = "assignment-indexing"
  ...
  override def process(envelope: EventEnvelope[AssignmentEntity.Event]): Future[Done] =
    envelope.event match {
      case evt: AssignmentEntity.PermissionAssigned   => indexDao.assignPermission(evt).map(_ => Done)
      case evt: AssignmentEntity.PermissionUnassigned => indexDao.unassignPermission(evt).map(_ => Done)
