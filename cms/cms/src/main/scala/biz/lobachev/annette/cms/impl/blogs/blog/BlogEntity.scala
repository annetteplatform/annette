...
object BlogEntity {
  ...
  def apply(entityContext: EntityContext[Command]): Behavior[Command] =
    apply(PersistenceId(entityContext.entityTypeKey.name, entityContext.entityId))
      .withTagger {
        case evt: BlogCreated => Set(tagger.tagFor(evt.id))
        case evt: BlogNameUpdated => Set(tagger.tagFor(evt.id))
        case evt: BlogDescriptionUpdated => Set(tagger.tagFor(evt.id))
        case evt: BlogCategoryUpdated => Set(tagger.tagFor(evt.id))
        case evt: BlogAuthorPrincipalAssigned => Set(tagger.tagFor(evt.id))
        case evt: BlogAuthorPrincipalUnassigned => Set(tagger.tagFor(evt.id))
        case evt: BlogTargetPrincipalAssigned => Set(tagger.tagFor(evt.id))
        case evt: BlogTargetPrincipalUnassigned => Set(tagger.tagFor(evt.id))
        case evt: BlogActivated => Set(tagger.tagFor(evt.id))
        case evt: BlogDeactivated => Set(tagger.tagFor(evt.id))
        case evt: BlogDeleted => Set(tagger.tagFor(evt.id))
      }
