...
object BlogCategoryEntity {
  ...
  val empty = BlogCategoryEntity(None)

  def apply(persistenceId: PersistenceId): EventSourcedBehavior[Command, Event, BlogCategoryEntity] =
    EventSourcedBehavior
      .withEnforcedReplies[Command, Event, BlogCategoryEntity](
        persistenceId = persistenceId,
        emptyState = BlogCategoryEntity.empty,
  ...
  implicit val categoryEntityFormat: Format[BlogCategoryEntity] = Json.format
}

final case class BlogCategoryEntity(maybeState: Option[model.BlogCategoryState]) {

  import BlogCategoryEntity._

  def applyCommand(cmd: Command): ReplyEffect[Event, BlogCategoryEntity] =
    cmd match {
  ...
  def createCategory(
    cmd: CreateCategory
  ): ReplyEffect[Event, BlogCategoryEntity] =
    maybeState match {
      case None    =>
  ...
  def getCategory(replyTo: ActorRef[Confirmation]): ReplyEffect[Event, BlogCategoryEntity] =
    maybeState match {
      case Some(state) => Effect.reply(replyTo)(SuccessCategory(state.toCategory))
      case None        => Effect.reply(replyTo)(NotFound)
    }

  def applyEvent(evt: Event): BlogCategoryEntity =
    evt match {
  ...
  def onCategoryCreated(event: CategoryCreated): BlogCategoryEntity =
    BlogCategoryEntity(
      Some(
        event
          .into[model.BlogCategoryState]
  ...
  def onCategoryUpdated(event: CategoryUpdated): BlogCategoryEntity =
    BlogCategoryEntity(
      Some(
        event
          .into[model.BlogCategoryState]
  ...
  def onCategoryDeleted(): BlogCategoryEntity =
    BlogCategoryEntity(None)
