...
object FileEntity {
  ...
  def apply(entityContext: EntityContext[Command]): Behavior[Command] =
    apply(PersistenceId(entityContext.entityTypeKey.name, entityContext.entityId))
      .withTagger {
        case evt: FileStored => Set(tagger.tagFor(s"${evt.objectId}-${evt.fileType}-${evt.fileId}"))
        case evt: FileRemoved => Set(tagger.tagFor(s"${evt.objectId}-${evt.fileType}-${evt.fileId}"))
      }
