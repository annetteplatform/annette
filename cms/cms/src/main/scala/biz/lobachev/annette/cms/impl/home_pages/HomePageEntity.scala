...
object HomePageEntity {
  ...
  def apply(entityContext: EntityContext[Command]): Behavior[Command] =
    apply(PersistenceId(entityContext.entityTypeKey.name, entityContext.entityId))
      .withTagger {
        case evt: HomePageAssigned   => Set(tagger.tagFor(HomePage.toCompositeId(evt.applicationId, evt.principal)))
        case evt: HomePageUnassigned => Set(tagger.tagFor(evt.id))
      }
