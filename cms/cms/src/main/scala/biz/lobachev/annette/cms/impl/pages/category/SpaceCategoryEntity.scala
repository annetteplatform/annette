...
object SpaceCategoryEntity {
  ...
  val empty = SpaceCategoryEntity(None)

  def apply(persistenceId: PersistenceId): EventSourcedBehavior[Command, Event, SpaceCategoryEntity] =
    EventSourcedBehavior
      .withEnforcedReplies[Command, Event, SpaceCategoryEntity](
        persistenceId = persistenceId,
        emptyState = SpaceCategoryEntity.empty,
  ...
  implicit val categoryEntityFormat: Format[SpaceCategoryEntity] = Json.format
}

final case class SpaceCategoryEntity(maybeState: Option[model.SpaceCategoryState]) {

  import SpaceCategoryEntity._

  def applyCommand(cmd: Command): ReplyEffect[Event, SpaceCategoryEntity] =
    cmd match {
  ...
  def createCategory(
    cmd: CreateCategory
  ): ReplyEffect[Event, SpaceCategoryEntity] =
    maybeState match {
      case None    =>
  ...
  def getCategory(replyTo: ActorRef[Confirmation]): ReplyEffect[Event, SpaceCategoryEntity] =
    maybeState match {
      case Some(state) => Effect.reply(replyTo)(SuccessCategory(state.toCategory))
      case None        => Effect.reply(replyTo)(NotFound)
    }

  def applyEvent(evt: Event): SpaceCategoryEntity =
    evt match {
  ...
  def onCategoryCreated(event: CategoryCreated): SpaceCategoryEntity =
    SpaceCategoryEntity(
      Some(
        event
          .into[model.SpaceCategoryState]
  ...
  def onCategoryUpdated(event: CategoryUpdated): SpaceCategoryEntity =
    SpaceCategoryEntity(
      Some(
        event
          .into[model.SpaceCategoryState]
  ...
  def onCategoryDeleted(): SpaceCategoryEntity =
    SpaceCategoryEntity(None)
