...
package biz.lobachev.annette.cms.impl.pages.category.dao
import akka.Done
...
class SpaceCategoryDbDao(
  ...
  private val categorySchema = quote(querySchema[Category]("space_categories"))
  ...
  def createTables(): Future[Done] = {
    ...
    for {
      _ <- session.executeCreateTable(
             CassandraTableBuilder("space_categories")
               .column("id", Text, true)
               .column("name", Text)
  ...
  def createCategory(event: SpaceCategoryEntity.CategoryCreated): Future[Done] = {
    val category = event
      .into[Category]
  ...
  def updateCategory(event: SpaceCategoryEntity.CategoryUpdated): Future[Done] = {
