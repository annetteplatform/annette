...
class SpaceCategoryIndexDao(client: ElasticClient, configPath: String)(implicit
  ...
  def createCategory(event: SpaceCategoryEntity.CategoryCreated) =
    createIndexDoc(
      event.id,
      "id"        -> event.id,
      "name"      -> event.name,
  ...
  def updateCategory(event: SpaceCategoryEntity.CategoryUpdated) =
    updateIndexDoc(
      event.id,
      "id"        -> event.id,
      "name"      -> event.name,
  ...
  def deleteCategory(event: SpaceCategoryEntity.CategoryDeleted) =
