...
object SpaceEntity {
  ...
  def apply(entityContext: EntityContext[Command]): Behavior[Command] =
    apply(PersistenceId(entityContext.entityTypeKey.name, entityContext.entityId))
      .withTagger {
        case evt: SpaceCreated => Set(tagger.tagFor(evt.id))
        case evt: SpaceNameUpdated => Set(tagger.tagFor(evt.id))
        case evt: SpaceDescriptionUpdated => Set(tagger.tagFor(evt.id))
        case evt: SpaceCategoryUpdated => Set(tagger.tagFor(evt.id))
        case evt: SpaceAuthorPrincipalAssigned => Set(tagger.tagFor(evt.id))
        case evt: SpaceAuthorPrincipalUnassigned => Set(tagger.tagFor(evt.id))
        case evt: SpaceTargetPrincipalAssigned => Set(tagger.tagFor(evt.id))
        case evt: SpaceTargetPrincipalUnassigned => Set(tagger.tagFor(evt.id))
        case evt: SpaceActivated => Set(tagger.tagFor(evt.id))
        case evt: SpaceDeactivated => Set(tagger.tagFor(evt.id))
        case evt: SpaceDeleted => Set(tagger.tagFor(evt.id))
      }
