...
private[impl] class SubscriptionTypeDbEventProcessor(
  ...
  override val projectionName: String = "subscriptionType-cassandra"
  ...
  override def process(envelope: EventEnvelope[SubscriptionTypeEntity.Event]): Future[Done] =
    envelope.event match {
      case evt: SubscriptionTypeEntity.SubscriptionTypeCreated => dbDao.createSubscriptionType(evt).map(_ => Done)
      case evt: SubscriptionTypeEntity.SubscriptionTypeUpdated => dbDao.updateSubscriptionType(evt).map(_ => Done)
      case evt: SubscriptionTypeEntity.SubscriptionTypeDeleted => dbDao.deleteSubscriptionType(evt).map(_ => Done)
