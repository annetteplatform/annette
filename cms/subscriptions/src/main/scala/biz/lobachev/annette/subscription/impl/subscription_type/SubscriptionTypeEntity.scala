...
object SubscriptionTypeEntity {
  ...
  def apply(entityContext: EntityContext[Command]): Behavior[Command] =
    apply(PersistenceId(entityContext.entityTypeKey.name, entityContext.entityId))
      .withTagger {
        case evt: SubscriptionTypeCreated => Set(tagger.tagFor(evt.id))
        case evt: SubscriptionTypeUpdated => Set(tagger.tagFor(evt.id))
        case evt: SubscriptionTypeDeleted => Set(tagger.tagFor(evt.id))
      }
