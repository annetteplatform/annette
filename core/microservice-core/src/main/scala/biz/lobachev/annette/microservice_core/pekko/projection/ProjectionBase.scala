 ...
 */
object ProjectionBase {

  /**
   * One-shot bootstrap for a Cassandra-backed service. Creates the projection offset
   * tables AND initializes the read journal (which, with `keyspace-autocreate` /
   * `tables-autocreate` enabled, creates the journal/snapshot keyspaces and the
   * `tag_views` / `all_persistence_ids` tables the eventsByTag projections read).
   * Without the read-journal init a fresh keyspace makes every projection fail with
   * "Keyspace ... does not exist" / "unconfigured table tag_views" until the first
   * entity write happens to initialize the journal plugin (verified at runtime in
   * slice 013). Idempotent.
   */
  def initAll()(implicit system: ActorSystem[_]): Future[org.apache.pekko.Done] = {
    import system.executionContext
    val journalInitialized =
      PersistenceQuery(system)
        .readJournalFor[CassandraReadJournal]("pekko.persistence.cassandra.query")
        .initialize()
    for {
      _ <- CassandraProjection.createTablesIfNotExists()
      _ <- journalInitialized
    } yield org.apache.pekko.Done
  }
}
