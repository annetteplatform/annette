  ...
  def run(): Future[Done] =
    ConfigSource.default
      ...
      .fold(
        ...
        config =>
          for {
            _ <- if (config.enableOrgRoles)
                   orgRoleLoader.load(config.orgRoles, config.createdBy)
                 else Future.successful(Done)
            _ <- if (config.enableCategories)
                   orgCategoryLoader.load(config.categories, config.createdBy)
                 else Future.successful(Done)
            _ <- if (config.enableOrgStructure)
                   orgStructureLoader.load(config.orgStructure, config.disposedCategory, config.createdBy)
                 else Future.successful(Done)
          } yield Done
      )
