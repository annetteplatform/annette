...
object PersonServiceMain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try app.run()(system)
      ...
      case NonFatal(e) =>
        System.err.println(s"\n\n@@@@ Startup of PersonServiceApp failed, terminating ActorSystem and exiting: ${e}\n\n")
        e.printStackTrace()
        system.terminate()
...
private[impl] class PersonServiceApp() {
  ...
  def run()(implicit system: ActorSystem[_]): Unit = {
    ...
    Await.result(personDbDao.createTables(), 30.seconds)
    ...
    val handler    = PersonServiceHandler(serviceApi, AnnetteGrpcExceptionMapping.serverHandlerOrDefault _)
